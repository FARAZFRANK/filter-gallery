...
if ( ! function_exists( 'ufg_normalize_filters_recursive' ) ) {
	/**
	 * Recursively normalizes filter titles/text so both properties are populated.
	 *
...
if ( ! function_exists( 'ufg_remove_blank_filters_recursive' ) ) {
	/**
	 * Recursively strips filters with a blank title/text from the filter tree.
	 *
	 * @param array $filters The filter tree, passed by reference.
	 * @return void
	 */
...
function ufg_activation() {
	// Current plugin version is recorded by the migration class rather than here, to ensure migrations run.
}
...
/**
 * Renders the main admin page body.
 */
function ufg_main() {
...
/**
 * Renders the Docs admin page body.
 */
...
/**
 * Renders the Free vs Pro comparison admin page body.
 */
...
/**
 * Renders the Manage Gallery admin page body.
 */
...
/**
 * Registers and enqueues the React admin app bundle and its localized data.
 *
 * @return void
 */
function ufg_enqueue_react_app() {
	...
	$current_gallery_data = null;

	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'ufg-manage-gallery' === $_GET['page'] && isset( $_GET['id'] ) ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		$default_5_filters = array(
			(object) array(
				'title'     => __( 'One', 'filter-gallery' ),
				'text'      => __( 'One', 'filter-gallery' ),
				'filterkey' => 'one',
				'icon'      => 'fas fa-camera',
				'color'     => '#38B2F6',
				'children'  => array(),
			...
			(object) array(
				'title'     => __( 'Two', 'filter-gallery' ),
				'text'      => __( 'Two', 'filter-gallery' ),
				'filterkey' => 'two',
				'icon'      => 'fas fa-image',
				'color'     => '#38B2F6',
				'children'  => array(),
			...
			(object) array(
				'title'     => __( 'Three', 'filter-gallery' ),
				'text'      => __( 'Three', 'filter-gallery' ),
				'filterkey' => 'three',
				'icon'      => 'fas fa-star',
				'color'     => '#38B2F6',
				'children'  => array(),
			...
			(object) array(
				'title'     => __( 'Four', 'filter-gallery' ),
				'text'      => __( 'Four', 'filter-gallery' ),
				'filterkey' => 'four',
				'icon'      => 'fas fa-heart',
				'color'     => '#38B2F6',
				'children'  => array(),
			...
			(object) array(
				'title'     => __( 'Five', 'filter-gallery' ),
				'text'      => __( 'Five', 'filter-gallery' ),
				'filterkey' => 'five',
	...
	wp_localize_script(
		...
		array(
			...
			'galleries'          => $galleries,
			'currentGalleryData' => $current_gallery_data,
			'nextId'             => ufg_get_next_id(),
			'selectFiltersText'  => __( 'Select Filters', 'filter-gallery' ),
...
/**
 * Gets the next available gallery ID.
 *
 * @return int The next gallery ID.
 */
...
/**
 * AJAX callback: saves a gallery's filters.
 */
function ufg_gallery_filters_callback() {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'add-filters' ) ) {
		...
		if ( ! function_exists( 'ufg_generate_random_string' ) ) {
			/**
			 ...
			 */
			function ufg_generate_random_string( $length = 7 ) {
		...
		if ( ! function_exists( 'ufg_add_missing_filter_keys' ) ) {
			/**
			 * Assigns a filterkey to any filter (or child filter) that is missing one.
			 *
			 * @param array $filter_items The filter tree, passed by reference.
			 * @return void
			 */
			function ufg_add_missing_filter_keys( &$filter_items ) {
				foreach ( $filter_items as &$item ) {
					// Check for missing filterkey in parent.
					if ( ! isset( $item->filterkey ) ) {
						$item->filterkey = strtolower( str_replace( ' ', '-', $item->text ) ) . '-' . ufg_generate_random_string();
					}
					...
					if ( isset( $item->children ) && is_array( $item->children ) ) {
						foreach ( $item->children as &$child ) {
							if ( ! isset( $child->filterkey ) ) {
								$child->filterkey = strtolower( str_replace( ' ', '-', $child->text ) ) . '-' . ufg_generate_random_string();
							}
						}
					}
				}
		...
		if ( is_array( $filters ) ) {
			...
			ufg_add_missing_filter_keys( $filters );
		}
...
/**
 * AJAX callback: renders the admin markup for a newly added gallery image.
 */
...
/**
 * Builds the filter <select> options markup for a gallery image.
 *
 * @param int   $ufg_attachment_id The attachment ID.
 * @param array $filters           The gallery's filter tree.
 * @param array $selected_filters  Filter keys currently selected for this image.
 * @return void
 */
...
/**
 * AJAX callback: saves a gallery's images and their metadata.
 */
function ufg_save_gallery_callback() {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'save-gallery' ) ) {
		...
		$ufg_image_id          = array();
		$ufg_image_title       = array();
		$ufg_image_alt         = array();
		$ufg_image_description = array();
		$ufg_image_url         = array();
		$ufg_image_filters     = array();
		...
		if ( isset( $ufg_image_id['ufg-attachment-id'] ) && is_array( $ufg_image_id['ufg-attachment-id'] ) ) {
			foreach ( $ufg_image_id['ufg-attachment-id'] as $ufg_id ) {
				$ufg_id = absint( $ufg_id );
				// Only allow writes to genuine attachment posts, never arbitrary post/page IDs.
				if ( 'attachment' !== get_post_type( $ufg_id ) ) {
					continue;
				}
...
/**
 * AJAX callback: renders the admin markup for a gallery's saved images.
 *
 * @param int $ufg_gallery_id The gallery ID (overridden by the posted 'id' field).
 * @return void
 */
function ufg_load_gallery_callback( $ufg_gallery_id ) {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'load-gallery' ) ) {
		die;
	// phpcs:ignore Universal.ControlStructures.DisallowLonelyIf.Found -- kept as nested if to avoid re-indenting the large PHP/HTML block below.
	} else {
		...
		if ( isset( $_POST['id'] ) ) {
			...
			foreach ( $ufg_gallery['ufg-attachment-id'] as $key => $value ) {
				...
				<li class="ufg-admin-image-item ufg-image-<?php echo intval( $ufg_attachment_id ); ?>"
					...
					<div class="ufg-admin-form-group">
						<?php
						$selected_filters = isset( $ufg_gallery['ufg-image-filters'][ $value ] ) ? $ufg_gallery['ufg-image-filters'][ $value ] : array();
...
/**
 * AJAX callback: saves a gallery's display settings.
 */
...
/**
 * AJAX callback: deletes one or more galleries.
 */
function ufg_remove_gallery_callback() {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ufg-remove-gallery' ) ) {
		...
		if ( isset( $_POST['ufg_gallery_id'] ) && isset( $_POST['do_action'] ) ) {
			...
			if ( 'multiple' === $ufg_do_action && is_array( $ufg_gallery_id ) ) {
...
/**
 * AJAX callback: clones a gallery, including its filters and settings.
 */
function ufg_clone_gallery_callback() {
	...
	if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ufg-clone-gallery' ) ) {
		...
		if ( isset( $_POST['ufg_gallery_id'] ) && isset( $_POST['ufg_gallery_counter'] ) ) {
			...
			$ufg_cloning_details = array(
				'ufg_gallery_id' => $ufg_gallery_id,
				'gallery_name'   => $new_ufg_gallery_name,
			);
...
/**
 * Renders the Import / Export admin page body.
 */
...
/**
 * AJAX callback: exports one or more galleries as JSON.
 */
...
/**
 * AJAX callback: imports a single gallery from exported JSON data.
 */
function ufg_import_gallery_callback() {
	...
	$skip_images = isset( $_POST['skip_images'] ) && '1' === $_POST['skip_images'];
...
/**
 * Registers and enqueues the frontend scripts and styles.
 */
