<?php
/**
 * Renders the filter navigation UI and computes per-filter image counts.
 *
...
if ( ! function_exists( 'ufg_filters' ) ) {
	/**
	 * Renders the filter navigation UI for a gallery.
	 *
	 * @param int   $ufg_gallery_id The gallery ID.
	 * @param array $ufg_filters    The gallery's filter tree.
	...
	function ufg_filters( $ufg_gallery_id, $ufg_filters, $ufg_gallery, $atts = array() ) {
		if ( is_array( $ufg_filters ) && count( $ufg_filters ) ) {
			...
			if ( ! function_exists( 'ufg_compute_filter_counts' ) ) {
				/**
				 * Recursively computes the unique image count for each filter, including its children.
				 *
				 * @param array $filters    The filter tree (or subtree) to process.
				 * @param array $images_map Filter key => image IDs assigned to that filter.
				 * @param array $counts     Filter key => image count, populated by reference.
			...
			$ufg_filter_style = ( isset( $ufg_setting['filter_style'] ) && 'dropdown' === $ufg_setting['filter_style'] ) ? 'dropdown' : 'buttons';

			if ( ! function_exists( 'ufg_fa_class_to_unicode' ) ) {
				/**
				 * Maps a Font Awesome icon class to its Unicode glyph for use in <option> text.
				 *
				 * @param string $icon_class The Font Awesome class, e.g. 'fas fa-filter'.
				 * @return string The Unicode glyph followed by a space, or an empty string if unmapped.
				 */
				function ufg_fa_class_to_unicode( $icon_class ) {
					if ( empty( $icon_class ) ) {
						return '';
					}
					$hex_map = array(
					...
					foreach ( $hex_map as $key => $hex ) {
						if ( strpos( $icon_class, $key ) !== false ) {
			...
			if ( 'dropdown' === $ufg_filter_style ) {
				...
				if ( ! function_exists( 'ufg_render_dropdown_options' ) ) {
					/**
					 * Recursively echoes <option> elements for the dropdown filter style.
					 *
					 * @param array    $filters               The filter tree (or subtree) to render.
					 * @param callable $get_count_html         Callback returning the count suffix markup for a filter key.
					 * @param int      $ufg_show_filters_icon  Whether to show filter icons (0 or 1).
					 * @param int      $depth                  Current nesting depth, used for indentation/level classes.
					 * @return void
					 */
					function ufg_render_dropdown_options( $filters, $get_count_html, $ufg_show_filters_icon, $depth = 0 ) {
						...
						foreach ( $filters as $filter ) {
							...
							$icon_unicode = '';
							if ( 0 !== $ufg_show_filters_icon && isset( $filter->icon ) && ! empty( $filter->icon ) ) {
